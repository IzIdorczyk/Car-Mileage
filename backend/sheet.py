...
filename = f"{cfg['month']} {cfg['year']} {cfg['car_name']} {cfg['car_register_numbers']}"
...
worksheet.set_margins(left= inch_to_cm(0.5), right= inch_to_cm(0.1), top= inch_to_cm(0.2), bottom= inch_to_cm(0))
...
#top of the register
def register_top(page, pages_needed, first_day_date, last_day_date, car_register_numbers, first_day_value, month, year):
...
# rows
def table_rows(rows_amount: int, start_at, distances, dates, month, year):
...
# footer of register
def register_footer(month, year):
...
register_top(cfg['page'], cfg['pages_needed'], cfg['first_day_date'], cfg['last_day_date'], cfg['car_register_numbers'], cfg['first_day_value'], cfg['month'], cfg['year'])
table_top()
table_rows(rows_needed, cfg['start_at'], generate_distances(), dates, cfg['month_nr'], cfg['year'])
table_end()
register_footer(cfg['month'], cfg['year'])
...
try:
    ...
    wb.WorkSheets[0]
    # Save
    wb.ExportAsFixedFormat(0, filepath_pdf)
...
os.rename(filepath_xls, filepath_xlsx)